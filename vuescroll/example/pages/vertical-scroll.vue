<template>
  <optional-demo class="scroll-view" :pullDown="pullDownRefreshObj" :pullUp="pullUpLoadObj" :scrollbar1="scrollbarObj" :title="$t('examples.normalScrollList')" :desc="$t('normalScrollListPage.desc')">
    ...
    <div slot="options">
      ...
      <div class="group">
        <input-option class="item" name="startY" :value="startY" @update:value="updateStartY"></input-option>
      </div>
    </div>
    
    <div class="scroll-list-wrap" slot="demo">
      <scroll ref="scroll" :data="items" :scrollbar="scrollbarObj" :pullDownRefresh="pullDownRefreshObj" :pullUpLoad="pullUpLoadObj" :startY="parseInt(startY)"
              @pullingDown="onPullingDown" @pullingUp="onPullingUp" @click="clickItem">
      </scroll>
    </div>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    components: {OptionalDemo, Scroll, SwitchOption, InputOption, SelectOption},
    watch: {
      scrollbarObj: {
        handler() {
          this.rebuildScroll()
      ...
      pullDownRefreshObj: {
        handler(val) {
          ...
          console.log(scroll) */ 
         /* Object.keys(scroll).forEach(item => {
         	console.log(item)
         }) */
          if (val) {
            scroll.openPullDown()
    ...
    methods: {
      ...
      onPullingDown() {
        // 模拟更新数据
        console.log('pulling down and load data--下拉刷新啦...')
        setTimeout(() => {
      ...
      onPullingUp() {
        // 更新数据
        console.log('pulling up and load data--上拉加载啦...')
        setTimeout(() => {
          if (this._isDestroyed) {
            return
          }
